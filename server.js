...
app.put('/account', isAuthenticated, async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query('SELECT accounts FROM users WHERE id = $1', [userId]);
    ...
    await pool.query('UPDATE users SET accounts = $1 WHERE id = $2', [JSON.stringify(accounts), userId]);
    
    // Actualizamos la sesión, si es necesario, para reflejar el cambio
    req.session.user.accounts = accounts;
    
    res.status(200).json({ message: 'Cuenta actualizada correctamente', accounts });
  } catch (error) {
    ...
    try {
      ...
      res.render('resumen', {
        ...
        ultimos_mensajes: [{
          ...
          destinatario: destinatarioUltimo,
