    ...
    if (historial.length > 0) {
      ...
      historial.forEach(({ fecha, tipo_mensaje, accion }) => {
        ...
        if (accion && accion.toLowerCase().includes('mensaje')) {
          ...
          mensajesPorTipo[tipo_mensaje] =
            (mensajesPorTipo[tipo_mensaje] || 0) + 1;
      ...
      const dayArray = Object.entries(mensajesPorDia).sort(
        (a, b) => b[1] - a[1]
      );
        ...
        try {
          ...
          if (!isNaN(fechaActualizacion.getTime())) {
            ...
            fechaExpiracion.setDate(
              fechaActualizacion.getDate() + cuenta.codigos_validez_dias
            );
          ...
          console.error(
            `Error calculando días restantes para ${cuenta.usuario}:`,
            dateError
          );
        }
      }
...
app.post('/auditoria/patrimonio', isAuthenticated, async (req, res) => {
  ...
  if (userRole !== 'admin' && userRole !== 'auditoria') {
    return res
      .status(403)
      .redirect('/auditoria/patrimonio?error=Acción no permitida');
  }
  ...
  if (!usuario || !tipo_cuenta) {
    return res.redirect(
      '/auditoria/patrimonio?error=Usuario y Tipo de Cuenta son obligatorios'
    );
  }
  ...
  try {
    if (cuenta_id) {
      ...
      await pool.query(queryText, [
        ...
        fechaActualizacionCodigos,
        validezDias,
        celular || null,
        fechaUltimaCargaCelular,
        contrasena_mail || null,
        dispositivo || null,
        vendedor || null,
        autentificador_cuenta || null,
        duracion_numero || null,
        link_original || null,
        cuenta_id,
      ]);
      res.redirect(
        '/auditoria/patrimonio?success=Cuenta actualizada correctamente'
      );
    } else {
      // --- Modo Agregar ---
      const queryText = `
      ...
      res.redirect(
        '/auditoria/patrimonio?success=Cuenta agregada correctamente'
      );
    }
  } catch (error) {
    ...
    if (
      error.code === '23505' &&
      error.constraint === 'patrimonio_cuentas_usuario_key'
    ) {
      res.redirect(
        '/auditoria/patrimonio?error=El nombre de usuario de Instagram ya existe'
      );
    } else {
      res.redirect(
        `/auditoria/patrimonio?error=Error al guardar la cuenta: ${error.message}`
      );
    }
  }
});
...
async function getDashboardData(startDate, endDate) {
  ...
  try {
    ...
    const vendedoresResult = await pool.query(
      'SELECT cuentas_asignadas FROM vendedores'
    );
    ...
    allAssignedUsernames = [
      ...new Set(allAssignedUsernames.map((u) => u.toLowerCase())),
    ];
  ...
  try {
    ...
    if (vendedorIds.length > 0) {
      const sqlQueryMes = `
          ...
          `;
      const desempenoMesResult = await pool.query(sqlQueryMes, [
        vendedorIds,
        firstDayOfMonth,
        lastDayOfMonth,
      ]);
    ...
    if (allAssignedUsernames.length > 0) {
      ...
      try {
        ...
        results.forEach((item) => {
          mongoMessageCounts[item.username] = item.count;
        });
      } finally {
        await client.close();
      }
    ...
    const vendedoresParaVista = vendedores.map((vendedor) => {
      ...
      if (
        vendedor.cuentas_asignadas &&
        Array.isArray(vendedor.cuentas_asignadas)
      ) {
        cuentasConDatos = vendedor.cuentas_asignadas.map((cuenta) => {
          const cuentaLower = cuenta.toLowerCase();
          const desempenoCuentaHoy = desempenoHoyMap[vendedor.id]?.[
            cuentaLower
          ] || { mensajes: 0, respuestas: 0 };
      ...
      if (objetivoMensual > 0) {
        progresoMensualPct = Math.min(
          100,
          Math.max(0, (mensajesEsteMes / objetivoMensual) * 100)
        );
      }
      return {
        ...vendedor,
    ...
    res.render('vendedores', {
      ...
      error: initialDashboardData.error
        ? `Error al cargar datos del dashboard: ${initialDashboardData.error}`
        : req.query.error,
      today: hoy,
...
app.get('/vendedores/dashboard-data', isAuthenticated, async (req, res) => {
  ...
  if (!from || !to || !dateRegex.test(from) || !dateRegex.test(to)) {
    return res
      .status(400)
      .json({ error: 'Fechas inválidas o faltantes (YYYY-MM-DD)' });
  }
  ...
  if (dashboardData.error) {
    return res.status(500).json({
      error: `Error al obtener datos del dashboard: ${dashboardData.error}`,
    });
  }
...
app.post('/vendedores', isAuthenticated, async (req, res) => {
  ...
  if (!nombre)
    return res.redirect('/vendedores?error=El nombre es obligatorio');
  let cuentasArray = [];
  // ... (procesar cuentas_asignadas a cuentasJson) ...
  if (cuentas_asignadas && typeof cuentas_asignadas === 'string') {
    cuentasArray = cuentas_asignadas
      .split(',')
      .map((c) => c.trim().toLowerCase())
      .filter((c) => c !== '');
  }
  const cuentasJson = JSON.stringify(cuentasArray);
  const cumplimiento = parseFloat(porcentaje_cumplimiento) || 0.0;
...
app.get('/vendedores/:id/historial', isAuthenticated, async (req, res) => {
  ...
  try {
    ...
    res.render('vendedor_historial', {
      vendedor: vendedor,
      historial: Object.values(historialAgrupado).sort((a, b) =>
...
app.get('/resumen/data', isAuthenticated, async (req, res) => {
  ...
  try {
    ...
    const fechasUnicas = Array.from(
      new Set([...Object.keys(mensajesPorDia), ...Object.keys(likesPorDia)])
    ).sort();
...
app.put('/api/flecha/mensajes', isAuthenticated, async (req, res) => {
  ...
  try {
    ...
    if (!Array.isArray(mensajes)) {
      return res
        .status(400)
        .json({ error: 'Formato inválido, se espera array de mensajes' });
    }

    const queryText =
      'UPDATE users SET mensajes_flecha = $1 WHERE id = $2 RETURNING mensajes_flecha';
...
app.get(
  '/api/blancos/cuentas',
  isAuthenticated,
  nichosController.getCuentasBlancos
);
app.post(
  '/api/blancos/cuentas',
  isAuthenticated,
  nichosController.postCuentasBlancos
);
...
app.post('/flujo/save', isAuthenticated, async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: 'Flujo guardado correctamente', flujo: rows[0] });
  } catch (error) {
...
app.post('/targets', isAuthenticated, async (req, res) => {
  ...
  if (!base_account || !target_accounts_list) {
    return res.redirect(
      '/targets?error=Cuenta base y lista de cuentas objetivo son obligatorios.'
    );
  }
  ...
  try {
    ...
    if (targetAccountsArray.length === 0) {
      return res.redirect(
        '/targets?error=La lista de cuentas objetivo está vacía.'
      );
    }
  } catch (processError) {
    console.error('Error processing target accounts list:', processError);
    return res.redirect(
      '/targets?error=Error procesando la lista de cuentas objetivo.'
    );
  }
  ...
  try {
    ...
    console.log(
      `Targets saved for user ${userId}: ${result.upsertedCount} inserted, ${result.modifiedCount} modified`
    );

    res.redirect(
      '/targets?success=Lista de cuentas objetivo guardada correctamente.'
    );
  } catch (error) {
...
app.post('/campaigns', isAuthenticated, async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      ...
      [
        `{${rows[0].id}}`,
        JSON.stringify({ active: status === 'active' }),
        req.session.user.id,
      ]
    );
...
app.put('/campaigns/:id', isAuthenticated, async (req, res) => {
  ...
  try {
    ...
    if (verifyQuery.rows.length === 0) {
      return res
        .status(403)
        .json({ error: 'No tienes permiso para modificar esta campaña' });
    }
...
app.get('/campaigns/:id/stats', isAuthenticated, async (req, res) => {
  ...
  try {
    ...
    if (campaignQuery.rows.length === 0) {
      return res
        .status(404)
        .json({ error: 'Campaña no encontrada o no tienes acceso' });
    }
    ...
    // Mensajes por día
    const messagesByDay = await collection
      .aggregate([
